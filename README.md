# 👋 Hi there! I'm Edmund!

**🎯 Currently working at JULO as a QA Engineer, contributing to various web and mobile application testing initiatives, and actively supporting both manual and automation QA efforts. | 7+ Years of Experience | Passionate About Quality**

Welcome to my GitHub! I'm a Quality Assurance Engineer with hands-on experience in both **manual** and **automation testing** for **web and mobile applications**. I'm currently on a journey to deepen my skills in **test automation, scripting, and continuous testing** to help teams ship confidently with speed and quality.

---

## 💼 What I Do

- 🔍 Manual Testing (Web, Mobile, API)
- 🤖 Automation with **Selenium + Python**, Postman, and Katalon
- 🔗 CI/CD Test Integration (GitHub Actions, Jenkins)
- 📲 Android & iOS app testing (Functional, UI/UX, Regression)
- 🧪 API Testing & Mocking (Postman, Swagger)
- 📊 Production Data Validation (SQL, DBeaver, Metabase)
- 💬 Team Collaboration (Jira, Zephyr, Confluence)

---

## 🛠️ Tech & Tools

![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=flat-square&logo=selenium&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white)
![Cypress](https://img.shields.io/badge/Cypress-17202C?style=flat-square&logo=cypress&logoColor=white)
![Jira](https://img.shields.io/badge/Jira-0052CC?style=flat-square&logo=jira&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![jQuery](https://img.shields.io/badge/jQuery-0769AD?style=flat-square&logo=jquery&logoColor=white)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

---

## 📚 Currently Learning

- Advanced Python scripting for data-driven testing  
- GitHub Actions for CI/CD automation  
- API contract testing (Schema & Mocking)


---

## 📫 Let’s Connect!

- 💼 [LinkedIn](www.linkedin.com/in/edmundus-valin-591403132)
- 📧 Email: valinedmundus@gmail.com   

Thanks for stopping by! I believe quality is not just a process—it's a mindset. Let's build better software together 🚀

---
